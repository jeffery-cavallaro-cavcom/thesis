...
simplifications) is found to be \colorable{k} is the chromatic number of \(G\).  As will be shown, the tradeoff of
...
The outer loop is guaranteed to complete because \(k\) will eventually be greater than or equal to \(n\).  Thus, by
Proposition \ref{prop:coloring3}, the current state of \(G\) is \colorable{k}, causing the called subroutine to
return true.
...
\item \label{step:common} Calculate the common number of neighbors between each vertex in \(G\), stopping if one
  vertex's neighborhood is found to be a subset of another.

\item \label{step:subset} If \(G\) has vertices \(u\) and \(v\) such that \(N(u)\subseteq N(v)\) then replace
...
\item \label{step:ubcalc} Calculate an upper bound for the minimum number of common neighbors for all vertices
  in \(G\):
  \[c=n-2-\frac{n-2}{k-1}\]

\item \label{step:ubcheck} If \(b>c\) then return false (Corollary \ref{cor:inter}).
...
\item \label{step:fail} Since neither of the assumptions in steps \ref{step:call1} and \ref{step:call2} hold,
  conclude that \(G\) is not \colorable{k} and return false.
\end{enumerate}
...
simplified by removing sufficent vertices (steps \ref{step:smallcalc}--\ref{step:subset}) or when the outer loop
has sufficiently incremented \(k\) (step \ref{step:incr}) such that \(n\le k\).

Steps \ref{step:smallcalc}--\ref{step:subset} of the called subroutine attempt to remove vertices to achieve a
...
Steps \ref{step:dencalc}--\ref{step:density} and \ref{step:select}--\ref{step:ubcheck} of the called subroutine
...
The remaining steps of the called subroutine, steps \ref{step:select2}--\ref{step:fail}, constitute the recursive
...
subgraph is still properly colored using at most four (in fact, three) colors.
...
\begin{proof}
  ...
  \item[Inductive Assumption:] Let \(\abs{X}=r\).
    ...
    Assume that \(G\) is \colorable{k} iff \(G-X\) is \colorable{k}.
...
Step \ref{step:subset} removes vertices whose neighborhoods are subsets of other vertices.  This is supported by
...
Steps \ref{step:select}--\ref{step:ubcheck} establish an upper bound for the minimum shared neighbor count between
...
If nothing more can be done in the preceding steps then steps \ref{step:select2}--\ref{step:fail} revert to the
...
then it can be concluded that the input graph was indeed \colorable{k}.  Otherwise, it can be concluded that the
input graph is not \colorable{k} and the called subroutine returns the state of \(G\) prior to the recursive calls
to the outer loop.
