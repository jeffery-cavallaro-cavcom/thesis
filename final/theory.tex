...
continually repeating the process on \(G[V(G)-S]\) until \(S=\emptyset\).  The resulting partition represents a
chromatic coloring of \(G\).  Part of this research compares the Christofides algorithm to the proposed algorithm.
...
\begin{proof}
  ...
  It is claimed that \(P\) is a path.
...
Therefore, we can state the conclusion of Proposition~\ref{prop:degree}:
...
\figurename~\ref{fig:clique}.  \(G\) has lots of \clique{2}s; however, none of them are maximal.  \(G\) has several
maximal \clique{3}s --- for example: \(G[\set{b,e,g}]\).  There is only one maximal \clique{4}:
...
is known to be NP-hard as well~\cite{mcdiarmid}.  Thus, there are many attempts in the literature to find a good
