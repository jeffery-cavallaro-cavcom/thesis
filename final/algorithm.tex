...
close the performance gap.  This section proposes a new Zykov-like algorithm that attempts to do just that.

The major advantages of the Christofides and Zykov algorithms are that they don't depend on the connectedness of a
graph, an example of a chromatic coloring is readily available, and the fact that the algorithms can be coded
...
An early version of this algorithm was first proposed by the author and his advisor in collaboration with a team of
mechanical engineering researchers from SUNY Buffalo \cite{cavallaro}.  It accepts a graph \(G\) as input and
provides \(\X(G)\) and a chromatic coloring as output and is composed of the following components:
\begin{enumerate}
\item A main routine that establishes \(G\) as the root of the state tree and loops on increasing values of \(k\).
...
\item The last-first with color interchange greedy coloring algorithm used by the main routine to determine an
  initial upper bound \(k_{max}\) for the chromatic number of \(G\) and a corresponding \coloring{k_{max}} of
  \(G\).
...
\item The Edwards Elphick algorithm used by the recursive subroutine to determine a lower bound for the chromatic
  number of the graph at each state in the Zykov tree.
\item A global value \(k_{adj}\) that is initially set to \(k_{max}\) but can be adjusted downward by the recursive
  subroutine.
\item A global list \(S\) of vertices removed by the recursive subroutine in the order removed.
\item A coloring routine called by the main routine to construct a chromatic coloring of \(G\) based on the results
  of the algorithm.
\end{enumerate}

The main routine, called subroutine, and coloring routine are summarized in the following sections.  A complete
...
The main routine accepts a graph \(G\) as input and returns \(\X(G)\) and a chromatic coloring.  It initially
computes lower and upper bounds for \(k\) using Bron Kerbosch and greedy last-first, respectively.  It then loops
on increasing values of \(k\), starting with the lower bound and going no further than the upper bound.  For each
value of \(k\), the recursive subroutine executes a modified Zykov algorithm to determine if \(G\) is \colorable{k}.
The first such successful return identifies \(\X(G)\) and then the coloring routine is called to construct the
final coloring.

The steps of the main routine are as follows:

\begin{enumerate}
\item\label{step:outer:bron} Use the Bron Kerbosch algorithm to compute a \(k_{min}\).

\item\label{step:outer:greedy} Use the greedy last-first algorithm to compute a \(k_{max}\) and a default
  \coloring{k_{max}}.

\item\label{step:outer:initkadj} Initialize \(k_{adj}\) to \(k_{min}\).

\item\label{step:outer:initr} Initialized the removed vertices list \(S\) to empty.

\item\label{step:outer:initk} Initialize \(k\) to \(k_{min}\).

\item\label{step:outer:match} If \(k=k_{max}\) then accept the coloring found in step~\ref{step:outer:greedy} and
  go to step~\ref{step:outer:done}.

\item\label{step:outer:call} Call the recursive subroutine to determine if \(G\) is \colorable{k}.  The recursive
  subroutine returns a possibly simplified \(G\) called \(G'\) and a boolean value \(R\) that reports the result of
  the test.  It may also adjust \(k_{max}\) downward and update \(S\).

\item\label{step:outer:result} If \(G\) is \colorable{k} (\(R=\) true) then go to step~\ref{step:outer:color}.

\item\label{step:outer:newg} Replace \(G\) with \(G'\).  As will be seen, doing this avoids needless reapplication
  of certain steps in the recursive subroutine.

\item\label{step:outer:incrk} Increment \(k\) and go to step~\ref{step:outer:match}.

\item\label{step:outer:color} Call the coloring routine to construct the final coloring based on the current state
  of \(G\) and the removed vertex list \(S\).
...
\begin{figure}[H]
  ...
  \scalebox{0.75}{
    \begin{tikzpicture}[>=latex']
      ...
      \node (bron) [draw,predproc,below=of start] {BRON \(G\)};
      \node (greedy) [draw,predproc,below=of bron] {GREEDY \(G\)};
      \node (kadjinit) [draw,process,below=of greedy] {\(k_{adj}=k_{max}\)};
      \node (sinit) [draw,process,below=of kadjinit] {\(S=\emptyset\)};
      \node (kinit) [draw,process,below=of sinit] {\(k=k_{min}\)};
      \node (kcheck) [draw,decision,below=of kinit] {\(k=k_{max}\)?};
      \node (call) [draw,predproc,below=of kcheck] {CALL \(G,k\)};
      \node (newg) [draw,process,below=of call] {\(G=G'\)};
      \node (iskcolor) [draw,decision,below=of newg] {\(R=\)TRUE?};
      \node (color) [draw,predproc,right=of iskcolor] {COLOR \(G,S\)};
      \node (kdone) [draw,terminal,right=of color] {RETURN \(k\)};
      \node (kinc) [draw,process,below=of iskcolor] {\(k=k+1\)};
      \node (belowinc) [coordinate,below=0.5cm of kinc] {};
      \node (leftinc) [coordinate,left=3cm of belowinc] {};
      \draw [->] (start) -- node [auto] {\(G\)} (bron);
      \draw [->] (bron) -- node [auto] {\(k_{min}\)} (greedy);
      \draw [->] (greedy) -- node [auto] {\(k_{max}\)} (kadjinit);
      \draw [->] (kadjinit) -- (sinit);
      \draw [->] (sinit) -- (kinit);
      \draw [->] (kinit) -- (kcheck);
      \draw [->] (kcheck) -| node [auto,pos=0.1] {YES} (kdone);
      \draw [->] (kcheck) -- node [auto] {NO} (call);
      \draw [->] (call) -- node [auto] {\(G',R,k_{adj},S\)} (newg);
      \draw [->] (newg) -- (iskcolor);
      \draw [->] (iskcolor) -- node [auto] {YES} (color);
      \draw [->] (color) -- (kdone);
      \draw [->] (iskcolor) -- node [auto] {NO} (kinc);
...
routine will terminate immediately.  Thus, the recursive subroutine is always called with \(k\ge2\).  Also note
that the main routine is guaranteed to terminate because \(k\) will eventually reach \(k_{max}\), or \(k\) will
eventually equal or exceed \(n\) due to subgraph simplifications applied by the recursive subroutine.  The latter
case causes the recursive subroutine to return true.
...
\begin{figure}[H]
  ...
  \scalebox{0.5}{
    \begin{tikzpicture}[>=latex']
      \node (start) [draw,terminal] at (0,0) {START};
      \node (sinit) [draw,process,below=of start] {\(S'=\emptyset\)};
      \node (donecheck) [draw,decision,below=of sinit] {\(n\le k\)?};
      ...
      \node (remnode) [draw,process,left=of nodecheck] {\(\begin{array}{c}G=G-X \\ S'=S'\cup X\end{array}\)};
      \node (join) [coordinate] at ($(remnode)-(2.5cm,0)$) {};
      \node (kadj) [draw,decision,below=of nodecheck] {\(n<k_{adj}\)?};
      \node (adjust) [draw,process,below=of kadj] {\(k_{adj}=n\)};
      \node (edwards) [draw,predproc,below=of adjust] {EDWARDS \(G\)};
      \node (bound) [draw,decision,below=of edwards] {\(\ell<k_{adj}\)?};
      \node (bfail) [draw,terminal,right=of bound] {RETURN \(G\),FALSE};
      \node (common) [draw,process,below=of bound] {Calculate \(\abs{N(u)\cap N(v)}\)};
      \node (subcheck) [draw,decision,below=of common] {\(N(u)\subseteq N(v)\)?};
      \node (remsub) [draw,process,left=of subcheck] {\(G=G\cdot uv\)};
      \node (mininter) [draw,process,right=2.5cm of done] {\(\displaystyle b=\min_{u,v\in V(G)}\abs{N(u)\cap N(v)}\)};
      ...
      \node (finduv) [draw,process,below=of intercheck] {\(\displaystyle \min_{uv\notin E(G)}\abs{N(u)\cap N(v)}\)};
      ...
      \node (sup2) [draw,process,right=3cmof check2] {\(S=S'\)};
      \node (done2) [draw,terminal,below=of sup2] {RETURN \(G\),TRUE};
      \node (fail) [draw,terminal,below=of check2] {RETURN \(G\),FALSE};
      \draw [->] (start) -- node [auto] {\(G,k\)} (sinit);
      \draw [->] (sinit) -- (donecheck);
      \draw [->] (donecheck) -- node [auto] {YES} (done);
      \draw [->] (donecheck) -- node [auto] {NO} (edgecalc);
      \draw [->] (edgecalc) -- (edgecheck);
      \draw [->] (edgecheck) -| node [auto] {YES} (bfail);
      ...
      \draw [->] (nodecheck) -- node [auto] {NO} (kadj);
      \draw [->] (kadj) -- node [auto] {YES} (adjust);
      \draw [->] (kadj) -- node [auto] {NO} ($(kadj)+(3cm,0)$) |- (edwards);
      \draw [->] (adjust) -- (edwards);
      \draw [->] (edwards) -- node [auto] {\(\ell\)} (bound);
      \draw [->] (bound) -- node [auto] {YES} (common);
      \draw [->] (bound) -- node [auto] {NO} (bfail);
      \draw [->] (common) -- (subcheck);
      \draw [->] (subcheck) -- node [auto] {YES} (remsub);
      \draw (remsub) -| (join);
      \draw [->] (subcheck) -- node [auto] {NO} ($(subcheck)-(0,3cm)$) -- ++(7cm,0) |- (mininter);
      \draw [->] (mininter) -- (intercalc);
      \draw [->] (intercalc) -- (intercheck);
      \draw [->] (intercheck) -- node [auto] {YES} (interfail);
      \draw [->] (intercheck) -- node [auto] {NO} (finduv);
      \draw [->] (finduv) -- (save1);
      \draw [->] (save1) -- (call1);
      \draw [->] (call1) -- node [auto] {\(G'',R\)} (check1);
      \draw [->] (check1) -| node [auto] {YES} (sup2);
      \draw [->] (check1) -- node [auto] {NO} (save2);
      \draw [->] (save2) -- (call2);
      \draw [->] (call2) -- node [auto] {\(G'',R\)} (check2);
      \draw [->] (check2) -- node [auto] {YES} (sup2);
      \draw [->] (sup2) -- (done2);
