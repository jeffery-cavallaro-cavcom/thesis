...
construct such a coloring.  However, this is only a P-time step.
...
\item A list \(S\) of vertices removed by the recursive subroutine in the order removed.
...
Instead of walking a single Zykov tree for the whole graph, the Wang technique is used to break the input graph
into smaller graphs by selecting a vertex that occurs in the least number of MISs.  Each of these MISs implies a
set of vertex contractions and edge additions that are applied to the input graph.  Thus, the modified Zykov
algorithm is applied in sequence to these separate trees for each \(k\) value.  The first successful return
identifies \(\X(G)\) and then the coloring routine is called to construct the final coloring.
