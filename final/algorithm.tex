...
The major advantages of the Christofides and Zykov algorithms described in the previous section are that they don't
depend on the connectedness of a graph, an example of a chromatic coloring is readily available, and the fact that
the algorithms can be coded rather easily to run on a computer.  Their major disadvantage is their high runtime
complexity, which is inherent to the chromatic number problem.
...
\item It has better runtime performance than the well-known algorithms.
\end{enumerate}
...
as output, and is composed of an outer loop on values of \(k\) and a recursive subroutine called by the outer loop
to determine if \(G\) is \colorable{k}.  The outer loop and called subroutine are summarized in the following
...
The outer loop accepts a graph \(G\) as input and returns \(\X(G)\).  It initially computes lower and upper bounds
for \(k\) using Bron Kerbosch and greedy last-first, respectively.  It then loops on increasing values of \(k\),
starting with the lower bound and going no further than the upper bound.  For each value of \(k\), the called
subroutine executes a modified Zykov algorithm to determine if \(G\) is \colorable{k}.  The first such successful
return identifies \(\X(G)\).
...
\item\label{step:outer:greedy} Use the greedy last-first algorithm to compute a \(k_{max}\).

\item\label{step:outer:initk} Initialize \(k\) to \(k_{min}\).

\item\label{step:outer:match} If \(k=k_{max}\) then go to step~\ref{step:outer:done}.
...
\begin{figure}[H]
  ...
  \scalebox{0.75}{
    \begin{tikzpicture}[>=latex']
      ...
      \node (bron) [draw,predproc,below=of start] {\(\text{Bron}(G)\)};
      \node (greedy) [draw,predproc,below=of bron] {\(\text{Greedy}(G)\)};
      \node (kinit) [draw,process,below=of greedy] {\(k=k_{min}\)};
      \node (kmatch) [draw,decision,below=of kinit] {\(k=k_{max}\)?};
      \node (kcheck) [draw,predproc,below=of kmatch] {CALL \(G,k\)};
      ...
      \draw [->] (greedy) -- node [auto] {\(k_{max}\)} (kinit);
      \draw [->] (kinit) -- (kmatch);
      \draw [->] (kmatch) -| node [auto,pos=0.1] {YES} (kdone);
      \draw [->] (kmatch) -- node [auto] {NO} (kcheck);
  ...
  \caption{The proposed algorithm outer loop.}
  \label{fig:outer}
\end{figure}

Note that in the case of the null or an empty graph, the upper and lower bounds for \(k\) will match and the outer
loop will terminate immediately.  Thus, the subroutine is always called with \(k\ge2\).  Also note that the outer
loop is guaranteed to terminate because \(k\) will eventually reach \(k_{max}\), or \(k\) will eventually equal or
exceed \(n\) due to subgraph simplifications applied by the called subroutine.  The latter case causes the called
subroutine to return true.
...
\begin{figure}[H]
  ...
  \caption{The proposed algorithm called subroutine.}
  \label{fig:called}
\end{figure}

Step~\ref{step:sub:check} is the success termination condition.  Success occurs when \(G\) is simplified by
removing sufficent vertices (steps~\ref{step:sub:smallcalc}--\ref{step:sub:subset}) and the outer loop has
...
possibly simplified \(G\) to the outer loop as a starting point for the next candidate value of \(k\).
...
\item (I\ref{step:sub:call1}) Recursively call the subroutine with \(G\cdot ad\), as shown in
  \figurename~\ref{fig:contract}.  Now, \(n=5\) and \(m=7\).
...
\item (O\ref{step:outer:result})\label{step:last} The called subroutine returned true, so conclude that the \(G\)
