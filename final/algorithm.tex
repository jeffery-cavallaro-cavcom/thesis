...
Instead of walking a single Zykov tree for the whole graph \(G\), the Wang technique (see
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \node (a) [unlabeled node=green!25!white] at (0,1.5) {};
    \node (b) [unlabeled node=red!25!white] at (0,1.0) {};
    \node (c) [unlabeled node=blue!25!white] at (0,0.5) {};
    \node at (0,-0.25) {\(N(v)\)};
...
Step~\ref{step:sub:subset} contracts vertices whose neighborhoods are subsets of other vertices.  This is supported
...
\begin{proof}
  ...
  Fo the converse, assume that \(G\cdot uv\) is \colorable{k} and let \(w\in V(G\cdot uv\) be the identified
...
Theorem~\ref{thm:subset} is demonstrated in \figurename~\ref{fig:subset}.
...
\begin{figure}[H]
  \begin{minipage}{2.75in}
    ...
    \begin{tikzpicture}
      \colorlet{c1}{green!25!white}
      \colorlet{c2}{blue!25!white}
      \colorlet{c3}{yellow!25!white}
      \colorlet{c4}{orange!25!white}
      \colorlet{c5}{red!25!white}
      \draw (0,0) circle (3);
      \node at (0,-2.75) {\(G\)};
  ...
  \begin{minipage}{2.75in}
    ...
    \end{tikzpicture}
  \end{minipage}
  \caption{Demonstration of \theoremname~\ref{thm:subset}.}
  \label{fig:subset}
\end{figure}
