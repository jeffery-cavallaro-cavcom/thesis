...
Zykov algorithms typically have three components:
\begin{enumerate}
...
\item A recursive subroutine that performs the depth-first search of the state tree and updates \(X\) as
  necessary.
\end{enumerate}
...
\item Return \(n(X)\) as the found chromatic number for \(G\) and \(X\) representing a chromatic coloring of \(G\).
\end{enumerate}

The recursive subroutine is called with a graph \(H\) and has access to \(X\).  The steps are as follows:

\begin{enumerate}
\item\label{step:zykov:check} If \(H\) is not complete then go to step~\ref{step:zykov:branch}.
\item If \(n(H)<n(X)\) then \(X=H\).
\item Go to step~\ref{step:zykov:done}.
\item\label{step:zykov:branch} Select any two nonadjacent vertices \(u\) and \(v\) in \(H\).
...
\item Recursively call this subroutine with \(H''\).
\item\label{step:zykov:done} Return.
\end{enumerate}

Since the number of states grows exponentially, there is a need to develop bounding conditions.  Branch and bound
algorithms of this type are called \emph{Zykov algorithms}.  In fact, McDiarmid~(1978)~\cite{mcdiarmid} shows that
even with bounding, pure Zykov algorithms have at least \(\BO(c^{n\sqrt{\log(n)}})\) complexity for some real
number constant \(c>1\), which is worse than exponential.  Thus pure Zykov algorithms will generally perform worse
than Christofides/Wang-type algorithms.

The next section examines a well-known example of a Zykov algorithm that will be compared to the proposed
algorithm.
