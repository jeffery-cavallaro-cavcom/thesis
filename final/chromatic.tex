...
\[\w^*(G)\le\w(G)\le\X(G)\]
where \(\w^*(G)\) is a lower bound estimate for the clique number of \(G\).  It is well-known that certain
triangle-free graphs with \(\w(G)=2\) can have arbitrarily high \(\X(G)\) (for an example, see Zhang~\cite{zhang});
however, for the general case the clique number is a suitable lower bound.
...
1, & v_iv_j\in E(G)
\end{cases}\]
Note that the degree of a vertex \(v_i\) can be calculated from \(A\) by summing the \(A_{ik}\) row entries or the
\(A_{ki}\) column entries, and \(\dmax(G)\) can be found from the maximum such sum.

The Edwards algorithm is as follows.  Let \(G\) be a graph or order \(n\):
\begin{enumerate}
\item Select (either lowest index or at random) a vertex \(v\in V(G)\) of maximum degree in \(G\)
  \((\deg(v)=\dmax(G))\) and let \(S=\set{v}\).
\item\label{step:edw:select} Select the vertex \(v_i\in V(G)\) such that \(v_i\notin S\) with the minimum index
