...
\begin{figure}[H]
  ...
  \caption{Last first greedy algorithm with interchange mean difference versus edge probability.}
  \label{fig:greedyerr}
\end{figure}

Although still P-time, the interchange portion of the algorithm is somewhat stepwise expensive.  A random graph
analysis was performed on \(1000\) graphs for various edge probabilities and graph order comparing upper bound
values from greedy coloring with and without color interchange.  The results for \(n=50\) are typical and are
shown in \tablename~\ref{tab:greedy}.
...
\begin{table}[H]
  ...
  \begin{tabular}{|c|c|c|}
    \hline
    edge prob (\%) & with (mean) & withwithout (mean) \\
    \hline
    10 & 4.323 & 4.379 \\
    \hline
    20 & 6.297 & 6.313 \\
