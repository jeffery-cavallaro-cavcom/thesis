...
In practice, when a problem requires a particular attribute, the presence of some corresponding relation \(\sR\) is
assumed and we say something like, ``vertex \(v\) has attribute \(a\),'' instead of the more formal, ``vertex \(v\)
has attribute \(\sR(v)\).''
...
The part consolidation graphs that act as the inputs to the two algorithms are labeled graphs, where the labels
represent the various functional requirements:
...
used to indicate that an edge should never be used/traversed.  Non-adjacent vertices can be assumed to actually be
connected by edges with either zero or infinite weight, depending on the problem.  An example graph with edge weights
is shown in Figure \ref{fig:exweight}.
...
\begin{definition}[Proper Coloring]
  A coloring \(c\) on a graph \(G\) is called \emph{proper} when no two adjacent nodes are assigned the same color:
...
The primary purpose of a \(k\)-coloring of a graph \(G\) is to distribute the vertices of \(G\) into \(k\)
so-called \emph{independent} (some possibly empty) sets:
