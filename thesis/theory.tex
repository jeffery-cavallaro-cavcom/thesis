...
As demonstrated by vertex \(c\) in Figure \ref{fig:exgraph}, there is no requirement that every vertex in a graph
...
\begin{proposition}
  \label{prop:coloring}
  Let \(G\) be a graph.  If \(G\) is \colorable{k} then \(G\) is \colorable{(k+1)}
\end{proposition}
...
An example of two walks of length \(4\) is shown in Figure \ref{fig:walks}.  \(W_1\) is an open walk because it
...
The following sections described of some special classes of graphs that are important to the execution of the
proposed algorithm.
...
Figure \ref{fig:clique}.  \(G\) has lots of \clique{2}s; however, none of them are maximal.  \(G\) has several
maximum \clique(3)s --- for example: \(G[\set{e,f,g}]\).  There is only one maximal \clique{4}:
...
Since a \clique{k} of a graph \(G\) needs at least \(k\) colors in any proper coloring of \(G\), this provides a
...
\begin{theorem}[Myers and Liu, 1970]
  \label{thm:clique}
  Let \(G\) be a graph of order \(n\) and size \(m\):
