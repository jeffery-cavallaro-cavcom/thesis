...
algorithm for assumed successive values of \(k\) and testing whether a graph \(G\) is \colorable{k}.  Since a
...
to those parts.  Since there are other well-known algorithms to find a \coloring{k}, the advantage of improved
...
The called subroutine accepts the current state of \(G\) of order \(n\) and size \(m\) and the current value of
\(k\) such that \(k\ge2\) as inputs.  It returns a possibly simplified version of \(G\) and a boolean value
indicating whether the input \(G\) is \colorable{k}.  Note that whenever \(G\) is modified all values are with
respect to the modified \(G\).  The steps of the called subroutine and their associated theorems are as follows:
