...
values of \(k\) and testing whether a graph \(G\) is \colorable{k}.  Since a candidate \(k\) value is known,
  ...
  \begin{center}
    \scalebox{0.75}{
      \begin{tikzpicture}[>=latex']
        ...
        \node (kcheck) [draw,predproc,below=of kinit] {CHECK \(k\)-COLORABLE};
...
Note that the outer loop is guaranteed to complete because \(k\) will eventually equal \(n\), which will cause the
called subroutine to return true.  This occurs when \(G\) (or one of its simplifications) has been reduced to a
complete graph, which requires a separate color for each vertex.  This fact is expressed by the following
proposition:

\begin{proposition}[Algorithm Success Condition]
  \label{prop:success}
